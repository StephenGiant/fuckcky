...
import com.nsk.app.bussiness.healthy.PhyProActivity
...
class HealthyProDialog : DialogFragment(){
...
//        Log.e("dialog","初始化")
        ...
        listenner!!.onInit(phoneEdittext,etCode)
        downTextView.setOnClickListener {
            if(phoneEdittext.text.toString()!=null&&phoneEdittext.text.toString().length>0) {
                val map = mapOf<String, String>(Pair("mobile", phoneEdittext.text.toString().trim()), Pair("validCodeType", "5"))
...
//            dismiss()
    ...
    override fun onDismiss(dialog: DialogInterface?) {
        ...
        if(activity is PhyProActivity){
            (activity as PhyProActivity ).onDismiss(dialog)
        }
    ...
    interface OnCommitListenner{
        ...
        abstract fun onInit(phoneEdittext:EditText,etCode:EditText)
    }
